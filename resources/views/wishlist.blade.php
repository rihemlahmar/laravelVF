@extends('inc.master')
@section('title','OnlineShop Wishlist')
<style>
    .flex{
        position: fixed;
    bottom: 0;
...
<style>
   @media print{
    .justify-content-end,.a,.menu{display:none;}
   }
                        ...
                        <td>
                            <div class="cart-info">
                                ...
                                <div>
                                    ...
                                    <small>
                                    <input type="text" value="{{ $carts->price }}" name="productprice[]" hidden="">Price: {{ $carts->price }}TND</small><br>
                                    <a href="{{ url('delete', $carts->id) }}">Delete</a>
                                </div>
                            </div>
                        </td>
                        <td><input type="text" value="{{ $carts->quantity }}" name="productquantity[]" hidden=""> {{ $carts->quantity }}</td>
                        <td>{{ $carts->price }} TND</td>
                    </tr>
                @endforeach
                <tr class="justify-content-end"><td><button onclick="window.print()" class="btn">Confirm Order</button></td></tr>
